...
supernew_dir = os.path.join(base_dir, "SUPERNEW")
...
# Scan for all JSON files in SUPERNEW
new_json_files = glob.glob(os.path.join(supernew_dir, "loker_data*.json"))
print(f"Found {len(new_json_files)} new JSON files in SUPERNEW/ to merge.")
    ...
    with open(filepath, "r", encoding="utf-8") as f:
        ...
        for item in items:
            ...
            if link in records_by_link:
                ...
                # Keep original "tanggal_ditemukan", or set to 2026-07-03 if not present
                if "tanggal_ditemukan" not in existing_record:
                    existing_record["tanggal_ditemukan"] = "2026-07-03"
